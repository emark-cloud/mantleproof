...
- [~] **T21** REST API — `/api/audit/{addr}` ✅ + `/api/health` ✅ (T7, 2026-05-20). `/api/feed`, `/api/cache`, `/api/queries` still scaffold 501 (gated on T29 cache-warmer + deploy-feed indexer).
- [ ] **T22** x402 middleware — USDC on Base, EIP-3009 transferWithAuthorization, replay protection
- [~] **T23** MCP server 3 tools ✅ implemented (2026-05-20): `getAudit` live (reads engine `/api/audit/{addr}` end-to-end, integrity ✓ verified for all 3 demo audits); `auditContract` cache-first ✅; `requestAudit` honest scaffold gated on T11 (refuses with isError=true rather than fabricating receipt). Build green, stdio smoke green. Publish to npm pending user `npm publish --access public` (CI does NOT publish; credentials never leave dev machine).
