...
**Path A locked.** Mantle ships canonical ERC-8004 Identity + Reputation registries (no Validation registry — not needed); MantleProof self-registers (`register()` is permissionless) and wraps the official identity. We deploy **5** contracts. Audit staking was deactivated on 2026-06-10 — `StakingPool.sol` was retired to the roadmap (it remains in-repo but is no longer deployed), and the registry was redeployed staking-free; the disputes layer remains, minus the economic slashing of audit stake. Path B (own registries) was abandoned.
...
**MantleProof contracts — current live deploy on Mantle mainnet 5000 (5 contracts; the registry was redeployed staking-free 2026-06-10, the rest from the T43 redeploy 2026-05-24, all Mantlescan-verified; source of truth is `contracts/deployments/mantle.addresses.json`):**

| Contract | Mainnet address | Purpose |
|---|---|---|
| `MantleProofRegistry.sol` | `0xcF3703BD76C64DA8a13461e820456d0576662aaf` | Append-only audit registry **+ disputes layer**. `submitAudit(target, tier, severity, rootHash, ipfsCID)` is **nonpayable** for both tiers (audits anchor for gas only — staking deactivated 2026-06-10, now roadmap), callable only by the immutable `oracleSigner`. `submitDispute`/`resolveDispute`; Tier 1 not disputable. `getAudit(target)` view. Public read, signed write — the credibility-purchasing invariant. |
...
| `StakingPool.sol` | *not deployed (roadmap)* | **Deferred post-hackathon (deactivated 2026-06-10).** The economic-security layer — 2 MNT per Tier 2 audit held for a 30-day window, `slashByDispute` (registry-only) → disputer on RETRACTED, `unlock(rootHash)` after `unlocksAt`, `claimExploit` — remains in the repo but is **not part of the live 5-contract stack**. The retired deployment was at `0x2E279f4cAE39B5d0Fa57e08D0d455Ec9f6080ee9`. |
...
Mantle Sepolia (5003) carries a parallel deploy of the live 5-contract stack for the smoke + pipeline rehearsal (Registry `0x11B39545…CCf65`; full set in `contracts/deployments/mantleSepolia.addresses.json`). `MockUSDC.sol` exists for tests only.
